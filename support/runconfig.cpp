	...
	for( int i = 1; i < aArgc; ++i )
	{
		...
		else if( ret == 2 )
		{
			...
			if( 0 == std::strcmp( "fbshift", name ) )
			{
				...
				{
					throw Error( "Error while parsing command line\n" 
						"Value '{}' not valid for --fbshift; expected unsigned integer\n"
						"Use --help to print available command line options", name );
				}
			...
			else if( 0 == std::strcmp( "geometry", name ) )
			{
				...
				{
					throw Error( "Error while parsing command line\n" 
						"Value '{}' not valid for --geometry; expected <width>x<height>\n"
						"Use --help to print available command line options", name );
				}
		...
		{
			throw Error( "Error while parsing command line\n" 
				"Expected flag, got unparsable mayhem '{}'\n"
...
namespace
{
	constexpr char const synopsis[] = R"(Synopsis: {0} [--<flag> [...]] [--<option>=<value> [...]]
...
Example:
  {0} --geometry=1920x1080 --fbshift=1
...
The framebuffer will consequently be magnified by a factor two.
	...
	{
		std::vprint_unicode( stdout, synopsis, std::make_format_args(aProgramName) );
	}
}
