...
GLuint Context::create_program_()
{
...

	{
		...
		if( GL_TRUE != status )
		{
			char const* info = "<no info>";
			if( !log.empty() )
				info = reinterpret_cast<char const*>(log.data());

			throw Error( "Shader program linking failed: \n{}\n", info );
		}

		if( !log.empty() )
			std::print( stderr, "Note: shader program linking log:\n{}\n", (char const*)log.data() );
	}
...
namespace
{
	GLuint compile_shader_( GLenum aShaderType, std::size_t aSourceLength, char const* aSource, char const* aShaderIdent )
	{
		...
		if( GL_TRUE != status )
		{
			...
			throw Error( "{} \"{}\" compilation failed:\n{}\n", shaderTypeName, aShaderIdent, info );
		}

		if( !log.empty() )
			std::print( stderr, "Note: {} \"{}\" log:\n{}\n", shaderTypeName, aShaderIdent, (char const*)log.data() );
...
#	if !defined(NDEBUG)
	...
	{
		std::print( stderr, "OpenGL Debug: {} [{}]: {}\n", severity_str_(aSeverity), type_str_(aType), aMessage ? (char const*)aMessage : "<no message>" );
